...
The user supplied the three creator forms on 2026-09-22. `assets/material-rules.json` is the shared machine source for all five platforms; `assets/package-rules.json` covers game upload size separately. Field snapshots are not permanent API contracts. Refresh official tooling/form requirements at use time.
...
| bilibili-toy | name, slug (required) | none established by supplied fields | poster + icon (one each) |
...
Use the example manifests in assets for shape only. Fill real project copy, file paths and provenance. The sample Xingxia game's name, story, monetization, cloud-save claims and selected tags are not reusable defaults. The displayed workshop-managed route is that example's origin; imported H5 should use the applicable existing-game upload route.

Xiaohongshu name and description are each ≤14; version syntax was not specified. Scene options are the supplied seven categories. Permission values in our manifest (`album`, `camera`, `microphone`, `storage`) are internal metadata labels, not asserted SDK identifiers: select only used capabilities, map through the current official workflow and verify whether the backend requires an explicit no-permission option. Do not request all permissions by default.
...
Xingxia cover follows the supplied narrower JPG/PNG formats; the previously verified 5 MB limit remains documented. 800×450 is recommended. Optional promotional images/videos share one total of eight, not eight of each. Images accept JPG/PNG/WebP ≤5 MB. Videos accept MP4/WebM/OGG ≤100 MB and ≤60 seconds; 1024×556 is a recommended ratio. Do not require a video or promotional+gameplay composition where this form does not. A video must contain a visual stream; audio-only OGG is not a promotional video.
